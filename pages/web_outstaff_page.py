...
class WebOutstaffPage(BasePage):
    ...
    def click_button_outsource(self):
        try:
            button_request = self.scroll_to_element(Locators.button_request)
   ...
   # переделываем метод
    ...
    def get_data_team_and_spec(self):
        ...
        for desc in descriptions:
            ...
            # Проверяем все
            found = any(
                review['project_type'].strip().lower() == desc['project_type'].strip().lower() and
            ...
            assert found, f"Данные из JSON не найдены на странице для: {desc['project_type']} | {desc['exp']} | {desc['level']} | {desc['price']}"
